...
void init_interpr( interpr_t* interp, program_t* pgm ) {
    clear_iter( &interp->iter, pgm );
    interp->tokp = 0;
    interp->currtok = TOK_EOL;
}
...
bool intp_fetchtok( interpr_t* interp ) {
    ...
    uint8_t tok = interp->currtok = *interp->tokp++;
    if ( tok == TOK_EOL ) {
        return false;
    }
    static const struct {
    ...
    for ( int i=0; littbl[i].tok; ++i ) {
        if ( littbl[i].tok == tok ) {
            if ( !littbl[i].read_fn( (const uint8_t**)(&interp->tokp), interp->param ) ) {
                return false;
            }
            return true;
        }
        ...
        switch ( tok ) {
            ...
            default:
                break;
            case TOK_DECLIT:
                interp->number = strtod( interp->param, 0 );
            NONDEC:
                interp->number = (double)((int64_t)(strtoull( interp->param, 0, base )));
                break;
        }
    }
